...
import { 
  FiPlayCircle, FiTrendingUp, FiCreditCard, FiUsers, 
  FiCheckCircle, FiShield, FiFileText, FiCamera, FiBarChart2, FiGlobe, FiShare2, FiAtSign
} from 'react-icons/fi';
    ...
    <div className="min-h-screen bg-[#f8fafc] font-sans selection:bg-indigo-200">
      ...
      <nav className="flex justify-between items-center px-8 py-5 max-w-7xl mx-auto border-b border-transparent">
        ...
        <div className="flex gap-4 items-center">
          <Link to="/login" className="text-sm font-bold text-indigo-700 hover:text-indigo-900 transition-colors">Log In</Link>
          <Link to="/register" className="px-5 py-2.5 bg-indigo-700 text-white rounded-lg text-sm font-bold hover:bg-indigo-800 transition-all shadow-md active:scale-95">
            Get Started
          </Link>
        </div>
      </nav>
      ...
      {/* --- HERO SECTION --- */}
      <section id="home" className="max-w-7xl mx-auto px-8 pt-16 pb-24 flex flex-col lg:flex-row items-center gap-16 relative">
        ...
        <div className="flex-1 relative w-full h-[550px] flex items-center justify-center">
           ...
           <div className="absolute w-[90%] h-[90%] bg-[#1e293b] rounded-[2rem] shadow-2xl -rotate-6 transition-transform hover:-rotate-3 duration-500 overflow-hidden border-8 border-slate-800">
              ...
              <div className="w-full h-full bg-[#0f172a] p-6 flex flex-col gap-4 opacity-90">
                 <div className="flex justify-between items-center mb-2">
                    <div className="text-slate-300 font-bold text-sm tracking-wider">Student Finance</div>
                    <div className="flex gap-2"><div className="w-2 h-2 rounded-full bg-slate-500"></div><div className="w-2 h-2 rounded-full bg-slate-500"></div></div>
                 </div>
                 <div className="flex-1 bg-slate-800/50 rounded-xl border border-slate-700 flex flex-col justify-end p-4 relative overflow-hidden">
                    ...
                    <svg className="absolute bottom-0 w-full h-1/2 left-0" preserveAspectRatio="none" viewBox="0 0 100 100">
                      <path d="M0,100 L0,50 Q10,40 20,60 T40,60 T60,40 T80,80 T100,20 L100,100 Z" fill="rgba(20, 184, 166, 0.4)" stroke="#14b8a6" strokeWidth="2" />
                    </svg>
                 </div>
                 <div className="flex-1 bg-slate-800/50 rounded-xl border border-slate-700 p-4 border-dashed relative">
                    ...
                    <svg className="absolute top-1/2 w-full left-0" preserveAspectRatio="none" viewBox="0 0 100 20">
                      <polyline fill="none" stroke="#6366f1" strokeWidth="2" points="0,10 20,15 40,5 60,18 80,8 100,12" />
                    </svg>
                 </div>
              </div>
           </div>
           ...
           <div className="absolute top-24 -left-6 bg-white p-4 rounded-xl shadow-xl z-20 flex items-center gap-4 animate-[bounce_6s_infinite]">
              <div className="w-10 h-10 bg-teal-100 text-teal-600 flex items-center justify-center rounded-full"><FiTrendingUp size={20} /></div>
              <div>
                <div className="text-xs font-bold text-slate-500">Savings Goal</div>
                <div className="text-sm font-extrabold text-slate-800">92% Reached</div>
              </div>
           </div>
           ...
           <div className="absolute bottom-20 -right-6 bg-white p-4 rounded-xl shadow-xl z-20 flex items-center gap-4 animate-[bounce_7s_infinite_1s]">
              <div className="w-10 h-10 bg-indigo-100 text-indigo-600 flex items-center justify-center rounded-lg"><FiCreditCard size={20} /></div>
              <div className="flex flex-col gap-1 w-24">
                <div className="text-xs font-bold text-slate-800">Tuition Fund</div>
                <div className="w-full h-2 bg-indigo-100 rounded-full overflow-hidden"><div className="w-[80%] h-full bg-indigo-600 rounded-full"></div></div>
              </div>
           </div>
        </div>
           ...
           <div className="grid grid-cols-1 md:grid-cols-3 gap-6 auto-rows-[minmax(200px,auto)]">
              ...
              <div className="md:col-span-2 bg-white rounded-3xl p-8 border border-slate-100 shadow-sm flex flex-col md:flex-row items-center gap-8 hover:shadow-md transition-shadow">
                 <div className="flex-1">
                    <div className="w-12 h-12 bg-indigo-50 text-indigo-600 rounded-xl flex items-center justify-center mb-6"><FiBarChart2 size={24} /></div>
                    <h3 className="text-2xl font-bold text-slate-900 mb-3">Deep Insight Analytics</h3>
                    <p className="text-slate-500">Visualize your spending patterns over the semester. Understand exactly where your money goes with our custom "Spending Pulse" metrics.</p>
                 </div>
                 <div className="flex-1 w-full bg-slate-900 h-64 rounded-2xl p-4 relative overflow-hidden flex items-end">
                    ...
                    <div className="w-full flex justify-between items-end gap-1 h-3/4 opacity-80">
                      {[10, 30, 20, 50, 40, 80, 60, 90, 70, 40, 60].map((h, i) => (
                         <div key={i} className="w-full bg-[#0ea5e9] rounded-t-sm" style={{height: `${h}%`}}></div>
                      ))}
                    </div>
                    {/* Overlay sweeping line */}
                    <svg className="absolute inset-0 w-full h-full" preserveAspectRatio="none" viewBox="0 0 100 100">
                      <polyline fill="none" stroke="#22c55e" strokeWidth="2" points="0,80 20,60 40,70 60,40 80,30 100,10" />
                      <polyline fill="none" stroke="#ef4444" strokeWidth="1" points="0,70 20,80 40,50 60,50 80,40 100,20" />
                    </svg>
                 </div>
              </div>
              ...
              <div className="bg-[#4338ca] text-white rounded-3xl p-8 shadow-lg flex flex-col hover:shadow-xl transition-shadow relative overflow-hidden group">
                 ...
                 <div className="w-12 h-12 bg-white/20 rounded-xl flex items-center justify-center mb-auto backdrop-blur-sm"><FiCamera size={24} /></div>
                 <div className="mt-16">
                   <h3 className="text-2xl font-bold mb-3">Receipt Scanning</h3>
                   <p className="text-indigo-100/90 leading-relaxed">Snap a photo and let our AI extract total, tax, and category automatically.</p>
                 </div>
              </div>

              {/* Feature 3 */}
              <div className="bg-white rounded-3xl p-8 border border-slate-100 shadow-sm hover:shadow-md transition-shadow">
                 <div className="w-12 h-12 bg-emerald-50 text-emerald-600 rounded-xl flex items-center justify-center mb-6"><FiCheckCircle size={24} /></div>
                 <h3 className="text-xl font-bold text-slate-900 mb-2">Smart Budgeting</h3>
                 <p className="text-slate-500 text-sm">Set limits for coffee, textbooks, and nightlife. Get notified before you overspend.</p>
              </div>

              {/* Feature 4 */}
              <div className="bg-white rounded-3xl p-8 border border-slate-100 shadow-sm hover:shadow-md transition-shadow">
                 <div className="w-12 h-12 bg-orange-50 text-orange-600 rounded-xl flex items-center justify-center mb-6"><FiFileText size={24} /></div>
                 <h3 className="text-xl font-bold text-slate-900 mb-2">Loan Management</h3>
                 <p className="text-slate-500 text-sm">Keep track of your student loans and interest rates without the headache.</p>
              </div>

              {/* Feature 5 */}
              <div className="bg-white rounded-3xl p-8 border border-slate-100 shadow-sm hover:shadow-md transition-shadow">
                 <div className="w-12 h-12 bg-blue-50 text-blue-600 rounded-xl flex items-center justify-center mb-6"><FiUsers size={24} /></div>
                 <h3 className="text-xl font-bold text-slate-900 mb-2">Shared Expenses</h3>
                 <p className="text-slate-500 text-sm">Split rent and groceries with roommates easily. No more awkward conversations.</p>
              </div>
           ...
           <div className="flex flex-col md:flex-row items-center gap-16">
              ...
              <div className="flex-1 space-y-6">
                 ...
                 <div className="space-y-4 pt-4">
                    ...
                    <div className="flex items-center gap-3 text-slate-700 font-semibold"><FiCheckCircle className="text-teal-500" size={20} /> No Ads, No Selling Data</div>
                    <div className="flex items-center gap-3 text-slate-700 font-semibold"><FiCheckCircle className="text-teal-500" size={20} /> Export to CSV/Excel for Reports</div>
                 </div>
              </div>
              ...
              <div className="flex-1 w-full bg-[#f1f5f9] p-10 rounded-3xl relative overflow-hidden border border-slate-200 shadow-sm">
                 ...
                 <div className="flex items-center gap-4 mt-8 relative z-10">
                    <div className="w-12 h-12 bg-teal-600 rounded-full flex items-center justify-center text-white border-2 border-white shadow-sm overflow-hidden">
                       <img src="https://api.dicebear.com/9.x/avataaars/svg?seed=AlexC&backgroundColor=14b8a6" alt="Alex Chen" className="w-full h-full" />
                    </div>
                    <div>
                       <div className="font-bold text-slate-900">Alex Chen</div>
                       <div className="text-sm text-slate-500">Economics Major, Year 3</div>
                    </div>
                 </div>
              </div>
           </div>
