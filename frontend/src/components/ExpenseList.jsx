...
const CategoryIcon = ({ category }) => {
  switch (category) {
    case "Academic Tools":
      return <FiBook size={14} className="text-primary-600" />;
    case "Dining & Food":
      return <FiCoffee size={14} className="text-emerald-600" />;
    case "Transport":
      return <FiTruck size={14} className="text-orange-600" />;
    case "Housing":
      return <FiHome size={14} className="text-indigo-600" />;
    case "Utilities":
      return <FiZap size={14} className="text-purple-600" />;
    default:
      return <FiBox size={14} className="text-slate-600" />;
  }
};

const CategoryPill = ({ category }) => {
  let colorClass = "bg-slate-100 text-slate-600";
  if (category === "Academic Tools")
    colorClass = "bg-primary-50 text-primary-600";
  else if (category === "Dining & Food")
  ...
  return (
    <span
      className={`px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider rounded-full ${colorClass}`}
    >
      {category}
    </span>
  );
...
const ExpenseList = ({ expenses, fetchExpenses, setExpenseToEdit }) => {
  ...
  const handleDownloadPdf = (data, fileTag = "all") => {
    ...
    doc.save(
      `expense-report-${fileTag}-${new Date().toISOString().split("T")[0]}.pdf`,
    );
    ...
    <div className="dashboard-card mt-6 p-0 overflow-hidden">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left border-collapse">
          <thead>
            <tr>
              <th className="px-6 py-4 text-xs font-bold text-slate-400 uppercase tracking-wider w-40">
                Date
              </th>
